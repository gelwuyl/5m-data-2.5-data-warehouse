...
*   **Star Schema:** The industry standard for most analytical designs. It minimizes query complexity and simplifies join paths by connecting dimension tables directly to a central fact table.
*   **Snowflake Schema:** Normalizes dimension tables into sub-layers (e.g., splitting a store dimension into store and city tables). 
    *   **When to use:** Normalization is beneficial when dealing with massive dimensions (e.g., millions of product records with highly repetitive attributes) where reducing data redundancy significantly improves storage efficiency.
    *   **Decision Criteria:** The choice between Star and Snowflake schemas depends on whether the storage and performance benefits of normalization outweigh the added complexity of writing and maintaining multi-join queries.

### 2. Slowly Changing Dimensions (SCD)
In real-world data warehousing, descriptive attributes (such as a customer's address or a store's region) change over time. Slowly Changing Dimensions (SCD) manage these historical changes:
*   **Type 1 (Overwrite):** Overwrites existing data with the new value. Only the latest state is preserved, and historical context is lost.
*   **Type 2 (Historical Rows - Most Common):** Creates a new record for every change. It tracks history using metadata columns such as **Start Date**, **End Date**, and an **Active/Current Flag** (or `dbt_valid_from` and `dbt_valid_to` in dbt).
*   **Type 3 (Previous Value Column):** Adds a new column to the existing record to capture the immediate previous state, allowing for simple "before and after" comparisons.
*   **Type 4 (History Table / Mini-Dimension):** Separates current operational data from historical records into different tables. The main dimension table only holds the current active records (keeping it small and fast), while a separate history table records all historical changes over time to optimize performance for queries that only require the latest state.

### 3. Surrogate Keys vs. Natural Keys
*   **Natural Keys (e.g., Supplier ID, Store Number):** These are the identifiers generated by the source operational systems.
*   **Surrogate Keys:** These are system-generated, unique identifiers (often hashes or auto-incrementing integers) created specifically for the data warehouse.
   
    * **Why Surrogate Keys are Essential:** In a Type 2 SCD, a single Natural Key (like Store #1234) will have multiple rows representing different historical versions. A surrogate key uniquely identifies a **specific version** of that store. This allows fact tables to join directly to the correct historical record at a specific point in time without requiring complex date-range logic during query execution.

### 4. Modern Data Architecture & Tooling (DPT)
Modern cloud data warehouses (like BigQuery) and lakehouses (like Databricks) have completely changed how we design and build data pipelines.

#### The Paradigm Shift: Then vs. Now
| Feature | Traditional Databases (RDBMS) | Modern Cloud Platforms (BigQuery/Databricks) |
| :--- | :--- | :--- |
| **Storage & Compute** | Tightly coupled (expensive disk space) | Decoupled (cheap storage, pay-only-for-compute) |
| **Data Layout** | Row-oriented (scans entire tables) | Columnar & Distributed (scans only requested columns) |
| **Modeling Goal** | **Snowflake Schema** (normalize to save space) | **Star Schema** (denormalize for query simplicity) |

#### Managing the Stack with Data Pipeline Tooling (DPT)
Because modern storage is cheap and tables are larger, we use tools like **dbt (Data Build Tool)** to manage pipeline complexity. dbt brings software engineering practices to SQL through four pillars:

1. **Orchestration:** Runs SQL models in the correct dependency order (e.g., snapshots $\rightarrow$ dimensions $\rightarrow$ facts).
2. **DevOps & Git:** Manages SQL as code, enabling version control, code reviews, and CI/CD.
3. **Data Lineage:** Automatically maps how raw data transforms into final tables.
4. **Automated Testing:** Runs quality checks (e.g., `unique`, `not_null`) to catch bugs before they reach dashboards.
